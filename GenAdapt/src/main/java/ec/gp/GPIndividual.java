...
package ec.gp;
import abc.def.genadapt.SearchRunner;
...
public class GPIndividual extends Individual
    ...
    public void setup(final EvolutionState state, final Parameter base, SearchRunner runner)
        {
        super.setup(state,base,runner);  // actually unnecessary (Individual.setup() is empty)
...
public String toGPString (){
    String result="";
    for (int x=0;x<trees.length;x++){
        result=result+trees[x].printTree2();
